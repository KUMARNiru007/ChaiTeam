...
const Batches = () => {
  ...
  const tabCounts = {
    live: batchesData.filter(batch => batch.status === 'ACTIVE').length,
    upcoming: 0,
    past: batchesData.filter(batch => batch.status === 'COMPLETED').length || 0
  };
  ...
  const filteredBatches = batchesData.filter(batch => {
    ...
    const matchesTab = activeTab === 'live' ? batch.status === 'ACTIVE' : 
                      activeTab === 'past' ? batch.status === 'COMPLETED' : false;
    ...
    onMouseLeave={(e) => {
      ...
      <div style={{ marginBottom: '1rem' }}>
        <div style={{ marginBottom: '0.25rem' }}>
          <span style={{ color: '#b3b3b3', fontSize: '0.875rem' }}>Created: </span>
          <span style={{ color: '#ffffff', fontSize: '0.875rem' }}>{new Date(batch.createdAT).toLocaleString()}</span>
        </div>
        <div>
          <span style={{ color: '#b3b3b3', fontSize: '0.875rem' }}>Updated: </span>
          <span style={{ color: '#ff4d4f', fontSize: '0.875rem' }}>{new Date(batch.updatedAT).toLocaleString()}</span>
        </div>
      </div>
    ...
    <div style={{ padding: '1.5rem' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '2rem' }}>
          ...
          `}</style>
          <p style={{ marginTop: '1rem', color: '#b3b3b3' }}>Loading batches...</p>
        </div>
      ) : error ? (
